...
import { Box, Button, InputAdornment, ListItemText, Menu, MenuItem, TextField, Typography } from "@mui/material";
...
export const Epics = () => {
  ...
  const [anchorFilterEl, setAnchorFilterEl3] = useState<null | HTMLElement>(null);
  const [filterSearchTerm, setFilterSearchTerm] = useState("");
  const [filterCheckedItems, setFilterCheckedItems] = useState<string[]>([]);

  const [anchorEditFieldsEl, setAnchorEditFieldsEl] = useState<null | HTMLElement>(null);
  const [editFieldsSearchTerm, setEditFieldsSearchTerm] = useState("");
  const [editFieldsCheckedItems, setEditFieldsCheckedItems] = useState<string[]>([]);

  const epics = useLoaderData() as Epic[];
  ...
  const handleToggleFilter = (item: string) => {
    ...
    if(!newChecked.includes('Select All') && newChecked.length === epics.length){
  ...
  const filterItems = ["Select All", ...epics.map(epic => epic.title)];
  ...
  const handleClickEditFields = (event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  const handleEditFieldsSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <FullHeightSection className="bg-white p-4 shadow rounded-lg flex flex-col font-poppins">
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingX: '12px', paddingY: '6px' }}>
        <Typography variant="h6" className="!text-lg !font-semibold">Epics</Typography>
        <Box className="flex flex-col gap-2">
        <Box sx={{ display: 'flex', alignItems: 'center', gap: '8px', gridTemplateColumns: '' }}>
                   {/* Search Bar */}
          ...
          <div className="flex justify-end items-end">
              ...
              >
                ...
                {filterItemsFiltered.length > 0 ? (
                  filterItemsFiltered.map((item, index) => (
                    <MenuItem
                      key={index}
            ...
            <Button variant="outlined"
              ...
              >
                ...
                {filteredEditFieldsMenuItems.length > 0 ? (
                  filteredEditFieldsMenuItems.map((item, index) => (
                    <MenuItem
                      key={index}
                      sx={{
                        fontFamily: "Poppins, sans-serif",
