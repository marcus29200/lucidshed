...
export const loader = (queryClient: QueryClient) => {
  return async ({ params }: LoaderFunctionArgs) => {
    ...
    return queryClient.fetchQuery(storyQuery(orgId, id));
  }
}
...
export const Story = () => {
  ...
  const [targetDate, setTargetDate] = useState(new Date(story.estimated_completion_date));
      ...
      <Box sx={{
        ...
        <Form method="post" style={{ display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
          <Grid container spacing={2} sx={{ flexGrow: 1 }}>
            <Grid item xs={8}>
              <TextField variant="outlined" size="small" margin="dense" fullWidth label="Title" id="title" name="title" value={title} onChange={e => setTitle(e.target.value)}></TextField>
              <TextField variant="outlined" size="small" margin="dense" fullWidth label="Description" id="description" name="description" multiline minRows={8}
            ...
            <Grid item xs={4}>
              ...
              <DatePicker label="Due Date" name="targetDate"
                value={targetDate} onChange={e => setTargetDate(e)}
                slotProps={{ textField: { variant: "outlined", size: "small", margin: 'dense', fullWidth: true } }}></DatePicker>
              <TextField value={estimate} onChange={e => setEstimate(e.target.value)} variant="outlined" size="small" margin="dense" fullWidth type="number" label="Estimate" id="estimate" name="estimate" />
              <FormControl sx={{ width: '100%', marginTop: '4px' }}>
                <InputLabel size="small" id="priority-label">Priority</InputLabel>
                <Select value={priority} onChange={e => setPriority(e.target.value)}
                  variant="outlined" size="small" margin="dense" fullWidth labelId="priority-label" label="Priority" defaultValue={"low"} id="priority" name="priority">{
              ...
              <FormControl sx={{ width: '100%', marginTop: '4px' }}>
                ...
                <Select value={status} onChange={e => setStatus(e.target.value)} variant="outlined" size="small" margin="dense" fullWidth labelId="status-label" label="Status" defaultValue={"not-started"} id="status" name="status">{
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
            <Button variant="contained" sx={{ backgroundColor: 'neutral.lightest', color: 'black' }} color="neutral" onClick={() => navigate('..', { relative: 'path' })}>Go back</Button>
