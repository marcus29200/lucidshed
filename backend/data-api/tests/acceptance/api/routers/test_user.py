...
async def test_should_register_user_without_sendgrid(mock_send, data_api: TestClient):
...
@patch("app.api.utils.SendGridAPIClient.send")
async def test_should_register_user_with_sendgrid(mock_send, data_api: TestClient):
...
async def test_should_mark_unauthorized_for_expired_token_in_db(data_api: TestClient):
    _, _, headers = await authenticate(data_api, create_org=False)
